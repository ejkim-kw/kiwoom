(function(root, factory){
  ...
  function getProgress(state){
    ...
    if(!meta || meta.steps.length < 2) return null;
    const current = meta.steps.findIndex(([step])=>step===state.step);
    return {labels:meta.steps.map(([,label])=>label), current:current < 0 ? 0 : current};
  }
  ...
  function transition(state, event){
    ...
    if(event.type==='ACCOUNT_AUTH'){
      if(!DATA.accounts.some(x=>x.id===event.account)) return {state, error:'조회할 계좌를 선택해 주세요.', field:'account'};
  ...
  return {MENU_KEYS, FLOW_META, DATA, shouldHandle, getTitle, getProgress, maskAccount, createState, transition};
});
